...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max: int = 16, loss_type: str = 'wiou_v3'):
        ...
        self.loss_type = loss_type  # 修正：使用传入的loss_type，而非硬编码
        # Focal-EIoU超参数（论文最优值）
        self.focal_gamma = 0.5  # 修正：工业缺陷场景γ=0.5最优，而非2.0
        self.focal_alpha = 0.25  # 正负样本平衡系数
        # WIoU v3超参数（论文最优值）
        self.wiou_alpha = 1.9
        self.wiou_delta = 3.0
    ...
    def forward(
        ...
        # 仅计算前景样本损失
        if not fg_mask.any():
            return torch.tensor(0.0).to(pred_bboxes.device), torch.tensor(0.0).to(pred_bboxes.device)
    ...
    # ==================== 3. WIoU v3 函数（严格对齐论文） ====================
    def wiou_v3(self, box1, box2, xywh=False, eps=1e-7):
        """
        计算 WIoU v3 损失（严格对齐论文公式）
        ...
        # -------------------------- 4. 计算离群度β（论文核心） --------------------------
        # 适配不同输入维度：如果是[B,N,1]则在dim=1求平均，否则在dim=0
        dim = 1 if len(iou_loss.shape) > 1 else 0
        avg_iou_loss = torch.mean(iou_loss, dim=dim, keepdim=True) + eps
        # 离群度β = 当前框IoU损失 / batch平均IoU损失
        beta = iou_loss / avg_iou_loss
        ...
        # 约束损失范围（防止极端值）
        wiou_v3_loss = wiou_v3_loss.clamp(0.0, 2.0)
...
class v8DetectionLoss:
    ...
    def loss(self, preds: dict[str, torch.Tensor], batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # if KL_LOSS_CONTAINER and len(KL_LOSS_CONTAINER) == 1 :
        #     kl_loss = KL_LOSS_CONTAINER[0]
        # else:
        #     kl_loss = 0
        # KL_LOSS_CONTAINER.clear()
        #
        # loss += (kl_loss * 0.0001)
