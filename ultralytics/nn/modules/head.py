...
def window_partition(x, window_size):
    ...
    x = x.permute(0, 2, 4, 3, 5, 1).contiguous()  # [B, nWh, nWw, ws, ws, C]
...
def window_reverse(windows, window_size, H, W, B):
    ...
    nWh = H // window_size
    nWw = W // window_size
    x = windows.view(B, nWh, nWw, window_size, window_size, C)
    x = x.permute(0, 5, 1, 3, 2, 4).contiguous()  # [B, C, nWh, ws, nWw, ws]
    x = x.view(B, C, H, W)
    return x
...
class SAFusionLite(nn.Module):
    ...
    def __init__(self, channels, num_heads=4, window_size=7, reduction=4):
        ...
        # 保留 attention output projection
        self.attn_proj = nn.Sequential(
            nn.Conv2d(channels, channels, 1, bias=False), nn.BatchNorm2d(channels), nn.SiLU()
        )
    ...
    def forward(self, x):
        ...
        BnW, N, _ = q_w.shape
        q_w = q_w.view(BnW, N, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        k_w = k_w.view(BnW, N, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        v_w = v_w.view(BnW, N, self.num_heads, self.head_dim).permute(0, 2, 1, 3)

        attn = (q_w @ k_w.transpose(-2, -1)) * self.scale
        attn = attn.softmax(dim=-1)
        out_w = attn @ v_w

        out_w = out_w.permute(0, 2, 1, 3).contiguous().view(BnW, N, C)
        ...
        wc = self.channel_branch(x)
        wl = self.local_branch(x)
        gate = self.sigmoid(wc + wl)
...
class WCAPGCRSAFusion(nn.Module):
    ...
    参数:
        channels: 输入通道
        num_prototypes: PGCR 原型数
        reduction: attention 压缩率
        use_delta: 是否只融合增量 (Fw-x, Fp-x)，默认 True 更稳
    """

    def __init__(self, channels, reduction=4, use_delta=True):
        super().__init__()
        self.use_delta = use_delta

        # 你之前已经实现过这两个模块
        self.wca = WCALite(channels, reduction=reduction, preserve_residual=True)
        # self.pgcr = PGCR(channels, reduction=reduction, use_norm=False)
        self.pgcr = PGCR_Recurrent(channels)

        self.fuse = nn.Sequential(nn.Conv2d(channels * 2, channels, 1, bias=False), nn.BatchNorm2d(channels), nn.SiLU())

        self.sa = SAFusionLite(channels, reduction=reduction)
...
class Detect(nn.Module):
    ...
    def forward_head(
        ...
        cls_refine: torch.nn.Module = None,
