...
class BaseTrainer:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        self.last, self.best = self.wdir / "last.pt", self.wdir / "best0.7880.pt"  # checkpoint paths
    ...
    def _setup_scheduler(self):
        ...
        if self.args.cos_lr:
            self.lf = one_cycle(1, self.args.lrf, self.epochs-self.args.warmup_epochs)  # cosine 1->hyp['lrf']
        else:
            self.lf = lambda x: max(1 - x / (self.epochs-self.args.warmup_epochs), 0) * (1.0 - self.args.lrf) + self.args.lrf  # linear
    ...
    def _do_train(self):
        ...
        # Do final val with best0.7880.pt
        self.final_eval()
        if RANK in {-1, 0}:
            if self.args.plots:
                self.plot_metrics()
    ...
    def save_model(self):
        ...
        if self.best_fitness == self.fitness:
            self.best.write_bytes(serialized_ckpt)  # save best0.7880.pt
    ...
    def final_eval(self):
        ...
        with torch_distributed_zero_first(LOCAL_RANK):  # strip only on GPU 0; other GPUs should wait
            if RANK in {-1, 0}:
                ...
                if model:
                    # update best0.7880.pt train_metrics from last.pt
